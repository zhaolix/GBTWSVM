...
class GBList:
    ...
    def get_r(self):
        ...
        #lambda x: x.r 是一个匿名函数，它接受一个参数 x，并返回 x.r
    ...
    def del_ball(self,purty=0.,num_data=0):
        #删除那些 purity 属性小于 purty 并且 num 属性小于 num_data 的球对象
        T_ball=[]
...
def generate_ball_data(data,pur,delbals):
    ...
    for i in range(len(gb.granular_balls)):
        ...
        balldata.append(a)
    # print(balldata[0])
    return balldata #[c,r,label]
def gen_balls(data,pur,delbals):
    # df=pd.read_csv(url,header=None)
    # data=df.values
    # print(data.shape)
    balls=generate_ball_data(data,pur=pur,delbals=delbals) #[c,r,label]
    R_balls=[]
    for i in balls:
        t_ball=[]
        t_ball.append(i[0])
